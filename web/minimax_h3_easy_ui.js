...
function isNodeInactiveForMedia(node) {
    if (!node) return true;
    // ComfyUI has several ways to make a node unavailable.  Collapsed nodes
    // are intentionally not included here: collapsing is only a layout action.
...
function isCustomImageLoader(node) {
    return nodeMatchesClass(node, "MiniMaxH3LoadImage", "MiniMax H3 图片加载器", "__h3ImageLoaderInstalled");
}

function isVideoFrameRateResolution(node) {
    return nodeMatchesClass(node, "VideoFrameRateResolution", "MiniMax H3 视频分辨率与帧率控制器", "__h3VideoProcessorInstalled");
}

function isVideoFolderLoader(node) {
    return nodeMatchesClass(node, "VideoFolderLoader", "MiniMax H3 视频加载器", "__h3VideoFolderLoaderInstalled");
}

function isAudioFolderLoader(node) {
    return nodeMatchesClass(node, "AudioFolderLoader", "MiniMax H3 音频加载器", "__h3AudioFolderLoaderInstalled");
}

function isMediaChainActive(node, mediaType, depth = 0, visited = new Set()) {
    if (!node || isNodeInactiveForMedia(node) || depth > 8 || visited.has(node)) return false;
    visited.add(node);
    if (mediaType === "image" && isCustomImageLoader(node)) {
        return Boolean(String(getWidget(node, "image")?.value || "").trim());
    }
    if (mediaType === "video" && isVideoFolderLoader(node)) {
        // The loader's unload button clears this exact widget.  Treat an
        // empty selection as no media so it vanishes from both @ and the
        // queued media_N inputs instead of failing later during validation.
        return Boolean(String(getWidget(node, "选择视频")?.value || "").trim());
    }
    if (mediaType === "audio" && isAudioFolderLoader(node)) {
        // Match the video/image unload behavior: an empty audio selection is
        // not a usable reference and must not remain in Media or @.
        return Boolean(String(getWidget(node, "选择音频")?.value || "").trim());
    }
    if (mediaType === "video" && isVideoFrameRateResolution(node)) {
        const upstream = upstreamPreviewNodes(node, "video");
        return upstream.length > 0 && upstream.some((source) => isMediaChainActive(source, "video", depth + 1, visited));
    }
    return true;
}

function isMediaLinkActive(node, link) {
    const graph = node?.graph || app.graph;
    const source = graph?.getNodeById?.(Number(link?.source_id));
    return isMediaChainActive(source, String(link?.media_type || "image").toLowerCase());
}

function effectiveMediaLinks(node) {
    return normalizeLinks(node).filter((link) => isMediaLinkActive(node, link));
}
...
function getVirtualMediaBridgeLink(node) {
    if (!isTarget(node)) return null;
    const graph = node.graph || app.graph;
    for (const link of effectiveMediaLinks(node)) {
...
function mentionOptions(node) {
    ...
    return orderedLinks.map((link) => {
        ...
        // Audio names are usually deliberately curated and audio has no
        // visual thumbnail. Always expose its filename so it remains
        // identifiable even when images/videos use the compact index mode.
        const useFilename = type === "audio" || mode === "filename";
        const label = useFilename ? truncateMentionLabel(fullLabel) : `${LABELS[type] || type}${ordinal}`;
        return {
            type,
            tag,
            token: `@${useFilename ? fullLabel : label}`,
            label,
            fullLabel,
            ordinal,
            referenceMode: useFilename ? "filename" : mode,
...
function firstExternalAbsolutePath(node) {
    for (const widget of node?.widgets || []) {
        const value = widget?.value;
        if (value && typeof value === "object") {
            if (typeof value.filename === "string" && isExternalAbsolutePath(value.filename)) return value.filename;
            if (typeof value.subfolder === "string" && isExternalAbsolutePath(value.subfolder)) return value.subfolder;
        } else if (typeof value === "string" && isExternalAbsolutePath(value)) {
            return value;
        }
    }
    return "";
...
function getNodeVideoSrc(node) {
    // A. widget.element 内真实 <video> 的 currentSrc（已加载源）
    // B. 其次 <video>.src
    for (const widget of node?.widgets || []) {
    ...
    // C. widget 值本身已是 http/https/blob URL
    for (const widget of node?.widgets || []) {
        const value = widget?.value;
        if (typeof value === "string" && /^https?:|^blob:/i.test(value.trim())) return value.trim();
    }
    // 外部绝对路径（E:\...、D:/...、UNC、file://）不允许伪造 /view URL，
    // 也不要假定拆出 subfolder 后 /view 就能访问。
    const external = firstExternalAbsolutePath(node);
    if (external) {
        console.warn(
            "[MiniMaxH3Easy] 检测到外部绝对路径，但上游 Loader 没有暴露浏览器可访问的视频 URL，无法生成视频缩略图",
            { node: sourceLabel(node), path: external }
        );
        return "";
    }
    // D. 仅 ComfyUI input 内的 filename / {filename, subfolder, type} 允许构造 /view URL
...
function installCustomImageLoaderNode(nodeType, nodeData) {
    if (String(nodeData?.name || "") !== "MiniMaxH3LoadImage") return;
    if (nodeType.prototype.__h3ImageLoaderInstalled) return;
    nodeType.prototype.__h3ImageLoaderInstalled = true;
    const attachUnloadButton = (node) => {
        const imageWidget = getWidget(node, "image");
        if (!imageWidget || node.__h3UnloadImageButton) return;
        const unloadButton = node.addWidget("button", "卸载图片", null, () => {
            imageWidget.value = "";
        ...
        unloadButton.tooltip = "清空当前图片选择并输出空值。";
        node.__h3UnloadImageButton = unloadButton;
    };
    const originalCreated = nodeType.prototype.onNodeCreated;
    nodeType.prototype.onNodeCreated = function onNodeCreatedH3ImageLoader() {
        const result = originalCreated?.apply(this, arguments);
        attachUnloadButton(this);
        return result;
    };
    const originalConfigured = nodeType.prototype.onConfigure;
    nodeType.prototype.onConfigure = function onConfigureH3ImageLoader(info) {
        const result = originalConfigured?.apply(this, arguments);
        setTimeout(() => attachUnloadButton(this), 0);
        return result;
    };
}

const videoThumbnailWarnTimes = new Map();
const VIDEO_THUMBNAIL_WARN_INTERVAL_MS = 30000;

function warnVideoThumbnailFailure(videoUrl, phase, detail = {}) {
    const key = `${videoUrl || ""}|${phase}`;
    const last = videoThumbnailWarnTimes.get(key) || 0;
    if (Date.now() - last < VIDEO_THUMBNAIL_WARN_INTERVAL_MS) return;
    videoThumbnailWarnTimes.set(key, Date.now());
    console.warn(`[MiniMaxH3Easy] 视频缩略图生成失败 (${phase})`, { videoUrl, ...detail });
}

function getVideoFrameThumbnail(videoUrl) {
    if (!videoUrl) {
        warnVideoThumbnailFailure("", "empty-url", { reason: "视频 URL 为空" });
        return "";
    }
    ...
    if (!isLikelyVideoUrl(videoUrl) && !/^blob:|^data:video\//i.test(videoUrl)) {
        warnVideoThumbnailFailure(videoUrl, "invalid-url", { reason: "不是可识别的视频 URL" });
        return "";
    }
    ...
    const fail = (phase, detail = {}) => {
        ...
        warnVideoThumbnailFailure(videoUrl, phase, detail);
        cleanup();
    };
    ...
    const captureFrame = () => {
        ...
        if (!video.videoWidth || !video.videoHeight) {
            seekNextSample("seek", "seek 后视频帧未就绪（宽高为 0）");
            return;
        }
        try {
            ...
            if (!dataUrl) {
                seekNextSample("decode", "drawImage 后 toDataURL 为空");
                return;
            }
            succeed(dataUrl);
        } catch (error) {
            seekNextSample("decode", `drawImage/canvas 失败: ${String(error)}`);
        }
    };

    const seekNextSample = (phase = "", detail = "") => {
        if (finished) return;
        if (sampleIndex >= sampleTimes.length) {
            fail(phase || "seek", detail
                ? { reason: detail, samples: sampleTimes.length }
                : { reason: "所有采样帧均不可用", samples: sampleTimes.length });
            return;
        }
        const nextTime = sampleTimes[sampleIndex++];
        try {
            if (Math.abs(Number(video.currentTime || 0) - nextTime) < 0.015) captureFrame();
            else video.currentTime = nextTime;
        } catch (error) {
            seekNextSample("seek", `设置 currentTime 失败: ${String(error)}`);
        }
    };

    // timeout 只是兜底，正常路径是 loadedmetadata → seeked → drawImage 成功即 succeed
    setTimeout(() => fail("timeout", { limitMs: 5000 }), 5000);
    video.addEventListener("loadedmetadata", () => {
        ...
        if (!Number.isFinite(duration) || duration <= 0) {
            ...
            sampleTimes = Array.from(new Set([
                Math.min(0.5, Math.max(0, duration * 0.12)),
                0.5,
                duration * 0.3,
                duration * 0.55,
                duration * 0.8,
        ...
        if (!video.videoWidth || !video.videoHeight) {
            fail("metadata", { reason: "loadedmetadata 后视频宽高为 0" });
            return;
        }
        seekNextSample();
    }, { once: true });
    video.addEventListener("seeked", captureFrame);
    video.addEventListener("error", () => fail("error", { code: video.error?.code, message: video.error?.message }), { once: true });
    video.load?.();
    return "";
}

function upstreamPreviewNodes(node, mediaType) {
    const graph = node?.graph || app.graph;
    const expected = String(mediaType || "image").toUpperCase();
    const result = [];
    for (const input of node?.inputs || []) {
        const inputType = getSlotType(input);
        if (inputType && !inputType.includes(expected)) continue;
        if (input?.link == null) continue;
        const link = getNativeGraphLink(graph, input.link);
        const sourceId = link?.origin_id ?? link?.originId ?? link?.from_id ?? link?.fromId;
        const source = link?.origin_node || link?.originNode || link?.fromNode || link?.sourceNode
            || graph?.getNodeById?.(Number(sourceId));
...
function sourcePreviewUrl(node, mediaType, depth = 0, visited = new Set()) {
    if (!node) return "";
    if (visited.has(node) || depth > 2) return "";
    visited.add(node);
    if (mediaType === "audio") return "";
    if (!isMediaChainActive(node, mediaType)) return "";
    if (mediaType === "video") {
        // 视频优先级：真实 <video> 源 → poster → getNodeVideoSrc 完整路径，
        // 不被普通 <img> / node.imgs 截胡。
        let poster = "";
        for (const widget of node.widgets || []) {
            const element = widget?.element;
            const video = element?.matches?.("video") ? element : element?.querySelector?.("video");
            if (!video) continue;
            if (video.currentSrc || video.src) return getVideoFrameThumbnail(video.currentSrc || video.src);
            if (!poster && video.poster) poster = video.poster;
        }
        if (poster) return poster;
        const videoSrc = getNodeVideoSrc(node);
        if (videoSrc) return getVideoFrameThumbnail(videoSrc);
    ...
    // Pass-through and adapter nodes often have no browser-facing preview of
    // their own.  Follow up to two IMAGE/VIDEO inputs for preview only; the
    // actual execution source remains the original connected node.
    for (const upstream of upstreamPreviewNodes(node, mediaType)) {
        const preview = sourcePreviewUrl(upstream, mediaType, depth + 1, visited);
...
function install() {
    ...
    style.textContent = `
      ...
      .h3-mention-chip {
        ...
        background: transparent; color: #2e7d32; font-family: inherit; font-size: var(--h3-prompt-text-size, 12px);
        font-weight: 500; line-height: inherit; letter-spacing: 0; user-select: text; cursor: text;
      }
      .h3-mention-chip[data-media-type="video"] { color: #4a9eff; }
      .h3-mention-chip[data-media-type="audio"] { color: #ffeb3b; }
      ...
      /* The menu is already opened by typing @, so a separate title only
         consumes vertical space.  In some light themes its text is nearly
         invisible while its inherited line height leaves a large blank gap. */
      .h3-mention-menu-title { display: none; }
...
app.registerExtension({
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        installCustomImageLoaderNode(nodeType, nodeData);
